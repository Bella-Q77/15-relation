...
const SearchOverlay: React.FC = () => {
  ...
  const results = useMemo(() => {
    ...
    if (!kw) return { entities: state.entities.slice(0, 20), relationships: [] };
  ...
  const hasResults = results.entities.length > 0 || results.relationships.length > 0;
              ...
              <div>
                <div
                  style={{
                    ...
                    borderTop: results.entities.length > 0 ? `1px solid ${tokens.border}` : 'none',
                  }}
                >
                  关系 ({results.relationships.length})
                </div>
