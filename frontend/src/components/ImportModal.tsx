...
const ImportModal: React.FC<ImportModalProps> = ({ visible, onClose }) => {
  ...
  const hasExistingData = state.entities.length > 0 || state.relationships.length > 0;
  ...
  const showImportConfirm = (
    ...
    if (!hasExistingData) {
      performImport(importType, data, 'overwrite', demoData);
      return;
    }
    ...
    confirm({
      ...
      content: (
        <div>
          <p>当前画布上已有 <strong>{state.entities.length}</strong> 个实体和 <strong>{state.relationships.length}</strong> 条关系。</p>
          <p>请选择新数据的导入方式：</p>
        </div>
      ),
      okText: '覆盖原有数据',
      ...
      onCancel() {
        performImport(importType, data, 'merge', demoData);
      },
    });
  };
  ...
  const performImport = async (
    ...
    try {
      switch (importType) {
        case 'json':
          await actions.importJSON(data, mode);
          message.success(`JSON数据导入成功（${mode === 'overwrite' ? '覆盖模式' : '合并模式'}）`);
          setJsonText('');
          break;
        case 'csv-entities':
          if (mode === 'overwrite') {
          ...
          await actions.importCSVEntities(data);
          message.success(`CSV实体数据导入成功（${mode === 'overwrite' ? '覆盖模式' : '合并模式'}）`);
          setCsvText('');
          break;
        case 'csv-relationships':
          ...
          await actions.importCSVRelationships(data);
          message.success(`CSV关系数据导入成功（${mode === 'overwrite' ? '覆盖模式' : '合并模式'}）`);
          setCsvText('');
          break;
        case 'demo':
          if (demoData) {
            await actions.importJSON(JSON.stringify(demoData), mode);
            message.success(`示例数据加载成功（${mode === 'overwrite' ? '覆盖模式' : '合并模式'}）`);
          }
          break;
      }
      onClose();
  ...
  const handleImportJSON = async () => {
    ...
    showImportConfirm('json', jsonText);
  };
  ...
  const handleImportCSV = async () => {
    ...
    showImportConfirm(importType, csvText);
  };
            ...
            children: (
              <div>
                ...
                <Button
                  ...
                  block
                  onClick={() => {
                    showImportConfirm('demo', '', demoData);
                  }}
                >
                  加载示例数据（关系网络分析场景）
                </Button>
