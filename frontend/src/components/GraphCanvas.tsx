...
const GraphCanvas: React.FC = () => {
  ...
  const graphinData: GraphinData = useMemo(() => {
    ...
    const nodes = visibleEntities.map((entity) => {
      ...
      const displayLabel = isCollapsed ? `${entity.label} (+${hiddenCount})` : entity.label;

      const calculateFontSize = (label: string, nodeRadius: number): number => {
        const charCount = label.length;
        if (charCount === 0) return 12;

        const nodeDiameter = nodeRadius * 2;
        const maxAvailableWidth = nodeDiameter * 0.65;
        const maxFontSize = Math.min(nodeRadius * 0.5, 12);
        const fontSizeByWidth = maxAvailableWidth / charCount;

        return Math.max(Math.min(Math.round(fontSizeByWidth), maxFontSize), 7);
      };

      const nodeRadius = finalSize / 2;
      const iconFontSize = calculateFontSize(displayLabel, nodeRadius);
