...
const GraphCanvas: React.FC = () => {
  ...
  const layoutConfig = useMemo(() => {
    ...
    const hasAllPositions = state.entities.length > 0 &&
      state.entities.every(e => e.x !== undefined && e.y !== undefined);

    if (hasAllPositions && state.layoutType === 'forceAtlas2') {
      return { type: 'preseted' };
    }
    ...
    return cfg;
  }, [state.layoutType, gpuAccel, size.w, size.h, state.entities]);
