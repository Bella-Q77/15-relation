...
const GraphCanvas: React.FC = () => {
  ...
  const graphinData: GraphinData = useMemo(() => {
    ...
    visibleEntities.forEach((e) => degreeMap.set(e.id, 0));
    state.relationships.forEach((r) => {
      if (visibleSet.has(r.source) && visibleSet.has(r.target)) {
    ...
    const nodes = visibleEntities.map((entity) => {
      ...
      let nodeSize = 20 + Math.min(degree * 4, 40);
      if (analysisType === 'sna') {
  ...
  useEffect(() => {
    ...
    const onNodeClick = (e: any) => {
      ...
      if (nodeId && !isPhantom(nodeId)) {
        if (state.selectingEntityTarget) {
          const entity = state.entities.find((en) => en.id === nodeId);
          if (entity) {
            if (state.selectingEntityTarget === 'source') {
              actions.setEntityForSource(entity);
            } else if (state.selectingEntityTarget === 'target') {
              actions.setEntityForTarget(entity);
            }
          }
        }
