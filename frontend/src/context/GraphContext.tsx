    ...
    go: {
      main: {
        App: {
          ...
          ImportJSONDataWithMode: (json: string, mode: string) => Promise<GraphData>;
...
interface GraphContextType {
  ...
  actions: {
    ...
    importJSON: (json: string, mode?: 'merge' | 'overwrite') => Promise<void>;
...
export function GraphProvider({ children }: { children: React.ReactNode }) {
  ...
  const importJSON = useCallback(async (jsonStr: string, mode: 'merge' | 'overwrite' = 'merge') => {
    const backend = getBackend();
    if (!backend) {
      try {
    ...
    try {
      const data = await backend.ImportJSONDataWithMode(jsonStr, mode);
      if (mode === 'overwrite') {
        dispatch({ type: 'CLEAR_DATA' });
      }
      dispatch({ type: 'SET_GRAPH_DATA', payload: data });
