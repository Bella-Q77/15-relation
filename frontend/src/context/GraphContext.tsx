    ...
    go: {
      main: {
        App: {
          ...
          ImportJSONDataReplace: (json: string) => Promise<GraphData>;
          ExportJSONData: () => Promise<string>;
          ImportCSVEntities: (csv: string) => Promise<GraphData>;
          ImportCSVEntitiesReplace: (csv: string) => Promise<GraphData>;
          ImportCSVRelationships: (csv: string) => Promise<GraphData>;
          ImportCSVRelationshipsReplace: (csv: string) => Promise<GraphData>;
...
interface GraphContextType {
  ...
  actions: {
    ...
    importCSVEntities: (csv: string, mode?: ImportMode) => Promise<void>;
    importCSVRelationships: (csv: string, mode?: ImportMode) => Promise<void>;
    clearData: () => Promise<void>;
...
export function GraphProvider({ children }: { children: React.ReactNode }) {
  ...
  const importJSON = useCallback(async (jsonStr: string, mode: ImportMode = 'append') => {
    const backend = getBackend();
    if (!backend) {
      try {
    ...
    try {
      let data: GraphData;
      if (mode === 'replace') {
        data = await backend.ImportJSONDataReplace(jsonStr);
      } else {
        data = await backend.ImportJSONData(jsonStr);
      }
      dispatch({ type: 'SET_GRAPH_DATA', payload: data });
    } catch (e) {
      console.error('导入JSON失败:', e);
  ...
  const importCSVEntities = useCallback(async (csv: string, mode: ImportMode = 'append') => {
    const backend = getBackend();
    if (!backend) return;
    try {
      let data: GraphData;
      if (mode === 'replace') {
        data = await backend.ImportCSVEntitiesReplace(csv);
      } else {
        data = await backend.ImportCSVEntities(csv);
      }
      dispatch({ type: 'SET_GRAPH_DATA', payload: data });
    } catch (e) {
  ...
  const importCSVRelationships = useCallback(async (csv: string, mode: ImportMode = 'append') => {
    const backend = getBackend();
    if (!backend) return;
    try {
      let data: GraphData;
      if (mode === 'replace') {
        data = await backend.ImportCSVRelationshipsReplace(csv);
      } else {
        data = await backend.ImportCSVRelationships(csv);
      }
      dispatch({ type: 'SET_GRAPH_DATA', payload: data });
    } catch (e) {
