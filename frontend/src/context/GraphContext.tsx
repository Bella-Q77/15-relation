...
interface State {
  ...
  selectingEntityTarget: 'source' | 'target' | null;
  selectedEntityForSource: Entity | null;
  selectedEntityForTarget: Entity | null;
}
...
type Action =
  ...
  | { type: 'SET_SELECTING_ENTITY_TARGET'; payload: 'source' | 'target' | null }
  | { type: 'SET_ENTITY_FOR_SOURCE'; payload: Entity | null }
  | { type: 'SET_ENTITY_FOR_TARGET'; payload: Entity | null }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'SET_SELECTING_ENTITY_TARGET':
      return { ...state, selectingEntityTarget: action.payload };
    case 'SET_ENTITY_FOR_SOURCE':
      return { ...state, selectedEntityForSource: action.payload, selectingEntityTarget: null };
    case 'SET_ENTITY_FOR_TARGET':
      return { ...state, selectedEntityForTarget: action.payload, selectingEntityTarget: null };
    case 'CLEAR_ENTITY_SELECTION':
      return { ...state, selectedEntityForSource: null, selectedEntityForTarget: null, selectingEntityTarget: null };
    default:
      return state;
  }
}
...
const initialState: State = {
  ...
  linkMode: false,
  linkSource: null,
  selectingEntityTarget: null,
  selectedEntityForSource: null,
  selectedEntityForTarget: null,
};
...
interface GraphContextType {
  ...
  actions: {
    ...
    importCSVEntities: (csv: string) => Promise<void>;
    importCSVRelationships: (csv: string) => Promise<void>;
    clearData: () => Promise<void>;
    ...
    startNeuroDB: () => Promise<string>;
    stopNeuroDB: () => Promise<string>;
    setSelectingEntityTarget: (target: 'source' | 'target' | null) => void;
    setEntityForSource: (entity: Entity | null) => void;
    setEntityForTarget: (entity: Entity | null) => void;
...
export function GraphProvider({ children }: { children: React.ReactNode }) {
  ...
  const importJSON = useCallback(async (jsonStr: string) => {
    const backend = getBackend();
    if (!backend) {
      try {
  ...
  const importCSVEntities = useCallback(async (csv: string) => {
    const backend = getBackend();
    if (!backend) return;
    try {
  ...
  const importCSVRelationships = useCallback(async (csv: string) => {
    const backend = getBackend();
    if (!backend) return;
    try {
  ...
  const setSelectingEntityTarget = useCallback((target: 'source' | 'target' | null) => {
    dispatch({ type: 'SET_SELECTING_ENTITY_TARGET', payload: target });
  }, []);

  const setEntityForSource = useCallback((entity: Entity | null) => {
    dispatch({ type: 'SET_ENTITY_FOR_SOURCE', payload: entity });
  }, []);

  const setEntityForTarget = useCallback((entity: Entity | null) => {
    dispatch({ type: 'SET_ENTITY_FOR_TARGET', payload: entity });
  }, []);

  const clearEntitySelection = useCallback(() => {
    dispatch({ type: 'CLEAR_ENTITY_SELECTION' });
  }, []);
