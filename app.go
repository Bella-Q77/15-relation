...
func (a *App) ImportJSONData(jsonStr string) GraphData {
	return a.ImportJSONDataWithMode(jsonStr, "merge")
}

func (a *App) ImportJSONDataWithMode(jsonStr string, mode string) GraphData {
	var data GraphData
	var modeLabel string
	if mode == "overwrite" {
		data, _ = a.graph.ImportJSONWithMode(jsonStr, ImportModeOverwrite)
		modeLabel = "覆盖模式"
	} else {
		data, _ = a.graph.ImportJSONWithMode(jsonStr, ImportModeMerge)
		modeLabel = "合并模式"
	}
	a.saveToFile()
	a.history.Record("import_data", "graph", "", "",
		fmt.Sprintf("%s - %s, %d个实体, %d条关系",
			actionLabel("import_data"), modeLabel, len(data.Entities), len(data.Relationships)))
	return data
}
